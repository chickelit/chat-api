...
export default class Friendship {
  public async handle({ request, response, auth }: HttpContextContract, next: () => Promise<void>) {
    ...
    const areFriends = ![
      await Database.query().from("friendships").where({ user_id: auth.user!.id, friend_id: userId }).first(),
      await Database.query().from("friendships").where({ user_id: userId, friend_id: auth.user!.id }).first()
